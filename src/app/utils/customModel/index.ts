...
import { createRegionIdFromPolygon } from "../../utils";
...
export const getColorByFieldKey = (colorForFields: Record<string, string>[], fieldKey: string) => {
    ...
    return colorMap ? colorMap[fieldKey] : "#d0d0d0";
};
...
export const uniqueByKeepFirst = (array: any[], key: any) => {
    let seen = new Set();
...
export const getOrder = (currentOrders: { [key: string]: number }, orderId: string): number => {
    const orderNumber = currentOrders ? currentOrders[orderId] : 0;
